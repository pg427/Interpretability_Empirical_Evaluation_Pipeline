...
def estimate_dataset_dbscan_eps(folds, percentile=80, min_samples=5):
    """
    Estimate one DBSCAN eps value for a dataset.

    Uses the distance to the min_samples-th nearest neighbor
    across all folds, then selects a percentile.
    """
    ...
    for fold in folds:
        scaler = fold["scaler"]
        _, Xn, _ = _prepare_measure_inputs(fold, mode=scaler)

        nn = NearestNeighbors(n_neighbors=min_samples)
        nn.fit(Xn)

        distances, _ = nn.kneighbors(Xn)

        # distance to the 5th nearest neighbor
        kth = distances[:, min_samples - 1]
        kth_distances.extend(kth)
...
def similarity_measure(
    ...
    *,
    ...
    dbscan_eps_percentile: float = 50,
    dbscan_min_samples: int = 5,
    force_recompute: bool = False,
):
    """
    Calculates SHAP explanation dissimilarity for method-dataset pairs.

    One constant DBSCAN eps is used for the dataset. If dbscan_eps is None,
    eps is estimated using the min_samples-th nearest neighbor distance
    across all folds.

    Lower values indicate more similar SHAP explanations among feature-space
    similar instances.
    """

    similarity_path = base_dir / dataset / f"{method}_fold_model_shap_similarity.joblib"

    if similarity_path.exists() and not force_recompute:
        return load_model(similarity_path)

    # Estimate one dataset-level eps if not provided
    if dbscan_eps is None:
        dbscan_eps = estimate_dataset_dbscan_eps(
            folds,
            percentile=dbscan_eps_percentile,
    ...
    for fold_idx, fold in enumerate(folds):
        ...
        # STEP 1: DBSCAN clustering in normalized feature space
        db = DBSCAN(
            eps=dbscan_eps,
            min_samples=dbscan_min_samples,
        )
        ...
        # STEP 2: Mean pairwise SHAP distances within valid clusters
        clusters = {}
        cluster_means = []
        ...
        for c in sorted(set(cluster_labels)):
            ...
            clusters[f"cluster-{int(c)}"] = {
                "indices": idx.tolist(),
                "size": int(len(idx)),
                "mean_pairwise_shap_dist": c_mean,
            }

        # No global fallback
        if len(cluster_means):
            similarity_score = float(np.mean(cluster_means))
            similarity_status = "computed_from_dbscan_clusters"
        else:
            similarity_score = float("nan")
            similarity_status = "no_valid_dbscan_clusters"

            clusters["no_valid_clusters"] = {
                "reason": f"No DBSCAN cluster with at least {dbscan_min_samples} instances.",
                "eps_used": float(dbscan_eps),
        ...
        fold_model_shap_similarity.append({
            ...
            "dbscan_eps_used": float(dbscan_eps),
